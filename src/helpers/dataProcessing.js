...
const getGroupedCountyData = (data) => {
  ...
  for (const point of data) {
    ...
    if (date !== point.test_date) {
      // when date changes, increment index and initialize All counters
      const AllPercentTemp =
        counties.All.percentPositive[index] /
        (Object.keys(counties).length - 1);
      counties.All.percentPositive[index] = round(AllPercentTemp, 4);
      index++;
      date = point.test_date;
      if (index > 7) {
        const temp7 =
          counties.All.rolling7Avg[index - 8] /
          (Object.keys(counties).length - 1);
        counties.All.rolling7Avg[index - 8] = round(temp7, 4);
        counties.All.rolling7Avg.push(0);
      }

      if (index > 14) {
        const temp14 =
          counties.All.rolling14Avg[index - 15] /
          (Object.keys(counties).length - 1);
        counties.All.rolling14Avg[index - 15] = round(temp14, 4);
        counties.All.rolling14Avg.push(0);
      }

      for (const prop of propsToIncrement) {
        counties.All[prop].push(0);
      }
    }
    if (!counties[county]) {
      counties[county] = {
    ...
    if (index > 6) {
      ...
      counties.All.rolling7Avg[index - 7] += county7Avg;
    }
    if (index > 13) {
      ...
      counties.All.rolling14Avg[index - 14] += county14Avg;
    }

    // update summary for whole region
    counties.All.newCases[index] += parseInt(point.new_positives);
    counties.All.totalTests[index] += parseInt(point.total_number_of_tests);
    counties.All.percentPositive[index] +=
      counties[county].percentPositive[index];
  }
  const AllPercentTemp =
    counties.All.percentPositive[index] / (Object.keys(counties).length - 1);
  counties.All.percentPositive[index] = round(AllPercentTemp, 4);
  index++;

  if (index > 7) {
    const temp7 =
      counties.All.rolling7Avg[index - 8] / (Object.keys(counties).length - 1);
    counties.All.rolling7Avg[index - 8] = round(temp7, 4);
    console.log(temp7);
  }
